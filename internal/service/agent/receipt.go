...
// 汇报锚定(§3.4 修订):主 Agent 看到合同才知道"当初承诺交付什么、什么算完成",
// 汇报结果时心里有对照(如实说"部分完成"),而非盲目转述子 Agent 结果;
// 但只作背景理解,汇报指令明确不展示自查过程/达标表格。
...
func BuildReportInstruction(tasks []*domainagent.AgentTask, fullArtifact bool) string {
	...
	b.WriteString("直接汇报结果本身,不要罗列完成标准的逐项验收过程或达标表格;")
	b.WriteString("任务未完成或失败时如实说明(缺了什么、卡在哪),不要粉饰。")
