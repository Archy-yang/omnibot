...
type SkillRepository interface {
	...
	// DeleteAllMCPSkills 清理全部 source=mcp 的技能行(MCP 工具目录已移入内存缓存,
	// 不再入库;此为目录化迁移的一次性清废)。
...
// DeleteAllMCPSkills 清理全部 source=mcp 的技能行(目录化后的一次性清废)。
func (r *skillRepository) DeleteAllMCPSkills() (int64, error) {
	res := r.db.Where("source = ?", skilldomain.SourceMCP).Delete(&skilldomain.Skill{})
