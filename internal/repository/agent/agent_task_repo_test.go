...
func TestAgentTaskRepository_CreateAndGetByID(t *testing.T) {
	...
	task := domain.NewAgentTask(42, "researcher", domain.NewTaskSpec("研究 Go 1.24 新特性"))
...
func TestAgentTaskRepository_UpdateStatus(t *testing.T) {
	repo := NewAgentTaskRepository(setupAgentTaskTestDB(t))
	task := domain.NewAgentTask(1, "researcher", domain.NewTaskSpec("goal"))
...
func TestAgentTaskRepository_ListCompletedUnreported(t *testing.T) {
	...
	t1 := domain.NewAgentTask(1, "researcher", domain.NewTaskSpec("g1"))
	t2 := domain.NewAgentTask(1, "researcher", domain.NewTaskSpec("g2"))
	t3 := domain.NewAgentTask(1, "researcher", domain.NewTaskSpec("g3"))
	t4 := domain.NewAgentTask(1, "researcher", domain.NewTaskSpec("g4"))
	...
	// 用户 2 的任务不应出现
	t5 := domain.NewAgentTask(2, "researcher", domain.NewTaskSpec("other"))
...
func TestAgentTaskRepository_ListByUser(t *testing.T) {
	repo := NewAgentTaskRepository(setupAgentTaskTestDB(t))
	for i := 0; i < 3; i++ {
		require.NoError(t, repo.Create(domain.NewAgentTask(1, "researcher", domain.NewTaskSpec("g"))))
	}
	require.NoError(t, repo.Create(domain.NewAgentTask(2, "researcher", domain.NewTaskSpec("other"))))
...
func TestAgentTaskRepository_Cancel(t *testing.T) {
	...
	task := domain.NewAgentTask(1, "researcher", domain.NewTaskSpec("g"))
	require.NoError(t, repo.Create(task))
...
func TestAgentTaskRepository_UpdateGoal(t *testing.T) {
	db := setupAgentTaskTestDB(t)
	repo := NewAgentTaskRepository(db)
	task := domain.NewAgentTask(1, "researcher", domain.NewTaskSpec("旧 goal"))
	require.NoError(t, repo.Create(task))
