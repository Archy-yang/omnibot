...
【怎么派】goal(必填)+ deliverables(交付物)+ completion_criteria(完成标准)是执行器判定
"做到什么程度算完"的唯一依据,委派前列清楚,缺了执行器只能靠直觉;persona_hint(可选)一句话定风格。
工具返回 task_id 后,一句话告诉用户"已安排处理X,稍后汇报",然后结束本轮——不要自己重复做执行器的事。`
