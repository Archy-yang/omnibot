...
> ⚠️ **校准注记（2026-09-25）**：v1.11.0 起进入未版本化快速迭代期（对话上下文 V1、后台 Agent 框架、
> MCP 连接器、订阅源、记忆重构等），未再逐版本记录。变更详情见 git log 与
> [16-架构迭代路线图-V1](../30-服务架构/01-高层设计/16-架构迭代路线图-V1-对话与上下文演进(草案).md) 的「落地进度」行。
...
- **Web 设置面板 Tab 化重构**:从单层平铺改为 3 Tab(模型 / 记忆 / 关于),
  modal 宽度 500 → 640px,视觉密度恢复舒适。
- **新增「关于」Tab**:
  - 显示 OmniBot 版本号(v1.10.0) + 标语(全平台智能助手)
  - 主题切换从 LLM 配置区域挪到这里,作为「全局偏好」语义更准
  - **三入口接入状态卡片**:Web / 微信 / 飞书 都已接入(✅),每条配标签 + 一句简介
  - 源码仓库链接(github.com/Archy-yang/omnibot)
- **底部按钮上下文化**:模型 Tab 显示「保存/取消」,记忆和关于 Tab 显示「关闭」
  (因为记忆是即时保存的 CRUD,关于是只读信息)
- **切 Tab 不丢未保存的 LLM 改动**:本地 ref 自然保留,切回模型 Tab 还能看到改动
- **每次打开默认进模型 Tab**:避免上次停留在「关于」造成困惑

### 🔧 架构改进

- 纯前端改造,后端零改动(无 API / DB schema 变化)
- 新增 `frontend/src/constants/about.ts`:版本号 / 应用名 / 标语 / 三入口接入状态常量 /
  源码链接——纯静态数据,不向后端要(后端没有比前端更权威的来源)
- `SettingsPanel.vue` 内嵌 `NTabs + NTabPane`,首次引入 Naive UI Tab 组件(Naive UI 自动导入,无新 npm install)
- `activeTab` ref 驱动底部按钮上下文化显示

### 🧪 测试

- `npm run build` 通过(含 `vue-tsc` 严格类型检查)
- 后端零改动,无新增 Go 测试需求
- 前端无单测框架(项目现状),依赖 build + tsc + 代码评审基线

### 📚 文档同步

- 演进路线图 v3.0:v1.10 章节重写为「Tab 化重构 + 关于页」,精准缩到本次实际做的
- 实际进度回顾表加 v1.10.0 节点
- 版本映射总览表更新 v1.10/v2.0 时间线

### ⚠️ 兼容性 / 范围

- **不动**:后端代码 / 业务逻辑 / API 路由 / 主题切换实现(只改位置)/ 数据库
- **明确不做**:
  - 跨入口主动绑定流程(生成验证码 → 微信/飞书端发码 → 合并 user_id)→ 留 v2.x 账户体系
  - 后端「用户配置汇总」单端点 API → 过度设计,LLM 配置/记忆按需独立加载已足够
  - 对话历史查看页面(本次决策选 Tab 重构方向,不走历史页方向)
  - 通道绑定 API / UI 展示(Web 用户的 user_id 与其微信 user_id 是独立的,
    展示「绑定了 1 个通道 = web」无价值,需要先有跨入口绑定流程才有意义)

### 🛡️ 安全

- 无加密路径变化,无敏感数据流转
- 关于 Tab 的「源码仓库」链接通过 `target="_blank" rel="noopener noreferrer"`,符合外链安全规范
