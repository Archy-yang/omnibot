...
| **上下文记忆集成** | `internal/api/wechat/handler.go` | ✅ 完成 | **对话时自动加载最近 10 轮历史消息，构建上下文传入 LLM** |
...
| 模块 | 文件 | 状态 | 说明 |
|------|------|------|------|
| **MessageService** | `internal/service/chat/message_service.go` | ✅ 完成 | **消息保存、上下文构建、历史消息查询、消息去重** |
| **上下文窗口管理** | `internal/service/chat/message_service.go` | ✅ 完成 | **自动维护最近 10 轮对话，超出窗口自动截断旧消息** |

### 7. 对话仓储层

| 模块 | 文件 | 状态 | 说明 |
|------|------|------|------|
| **MessageRepository** | `internal/repository/chat/message_repo.go` | ✅ 完成 | **消息 CRUD、按用户查询历史消息、MsgID 唯一索引去重** |
...
| **`messages`** | ✅ 完成 | **对话消息历史（用户消息 + AI 回复），MsgID 唯一索引去重** |
...
| **对话上下文记忆** | ✅ 完整 | **v1.2 版本：最近 10 轮对话记忆，永久落库，自动去重** |
| 向量检索 | ⏳ 待开发 | v1.3 版本 |
| H5 配置页面 | ⏳ 待开发 | v2.0 版本 |
...
用户微信公众号 → 微信服务器 → 我们的服务
    ↓
签名验证通过 → 解析 XML 消息 → 消息去重校验（MsgID 唯一索引）
    ↓
获取/创建用户 → 保存用户消息到数据库
    ↓
    ├───────────────────────────────────────────┐
...
检查用户是否有自定义 LLM 配置
    ...
    ↓
LLM 生成回答 → 保存 AI 回复到数据库
    ↓
转换为微信 XML 格式 → 返回给用户
```

> ⚠️ **校准注记（2026-09-25）**：本文件为早期进度笔记，版本状态已过期（现行状态见 CLAUDE.md「项目当前状态」与 doc 16 落地进度）。
>
> **当前状态 v1.2（历史）**：微信接入 + 用户体系 + LLM 对话 + **用户自定义 LLM 配置** + **对话上下文记忆** 全部完成，可投入生产使用。下一步迭代方向：向量检索增强记忆。
